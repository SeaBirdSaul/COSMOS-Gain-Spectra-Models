...
def featureExtraction_ML(data_characterization,extractionType,channelType,featureType,channelNum=95,calculateRipple=False,\
    ...
    for metadata in data_characterization:
        ...
        # gain profile
        actual_gain_spectra = EDFA_output_spectra - EDFA_input_spectra
        if calculateRipple:
            active_indices = np.where(np.array(wss_channels) - 1 >= 0)[0]
            if len(active_indices) > 0:
                active_mask = np.zeros(channelNum, dtype=bool)
                active_mask[np.array(wss_channels) - 1] = True
                active_gain = actual_gain_spectra[active_mask]
                actual_gain_spectra = actual_gain_spectra - np.mean(active_gain)
        ...
        for i in range(channelNum):
            ...
            metaResult['calculated_gain_spectra_'+post_indx] = actual_gain_spectra[i]
    ...
    for fileName in fileList:
        for edfaType in edfaTypes: # 
            ...
            if not os.path.exists(modelSubPath):
                os.makedirs(modelSubPath, exist_ok=True)
