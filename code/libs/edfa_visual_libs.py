...
def gain_to_ripple(gain_values):
    """Convert a list or array of active gain values into ripple around the active-mean."""
    gain_array = np.asarray(gain_values, dtype=np.float64)
    if gain_array.size == 0:
        return gain_array
    mean_gain = np.mean(gain_array)
    return gain_array - mean_gain
...
def calculateGainDict(data_characterization,label="booster",calculateRipple=False):
    ...
    for metadata in data_characterization:
        ...
        if calculateRipple:
            sample_gains = gain_to_ripple(sample_gains)

        for indx, gain in zip(sample_indices, sample_gains):
...
def plotRippleData(indxForFigure,dataDict,title,
        saveName=None,scatterColor="blue",alphaNum=0.2,
        gridLine=True,makerSize=2):
    plotGainData(indxForFigure,dataDict,title,
        saveName=saveName,scatterColor=scatterColor,alphaNum=alphaNum,
        gridLine=gridLine,makerSize=makerSize,ylabel='Ripple (dB)')
...
def plotGainData(indxForFigure,dataDict,title,
        saveName=None,scatterColor="blue",alphaNum=0.2,
        gridLine=True,makerSize=2,ylabel='Gain ripple (dB)'):
